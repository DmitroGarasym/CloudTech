...
SUBNET_1_ID=`aws ec2 describe-subnets --query 'Subnets[0].SubnetId' --filters Name=vpc-id,Values=$VPC_ID --output text`
SUBNET_2_ID=`aws ec2 describe-subnets --query 'Subnets[1].SubnetId' --filters Name=vpc-id,Values=$VPC_ID --output text`
...
touch ./MyKeyPairLB.pem
chmod 777 MyKeyPairLB.pem
...
# Retrieve IMAGE_ID
IMAGE_ID=`aws ec2 describe-images --owners 701577966315 --query Images[0].ImageId --output text`
...
INSTANCE_1_ID=`aws ec2 run-instances \
    ...
    --user-data file://user_script2.sh \
